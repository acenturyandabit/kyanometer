...
def drawBoxesDebug(forTD, drawOn, boxes, ratioWidth, ratioHeight, color=(0, 255, 0), width=1):
    ...
    for box in boxes:
        ...
        # Remove potential error 
        if any(idx<boundary for idx in [startX,startY]) == False:
            text = forTD[ startY - boundary:endY + boundary, startX - boundary:endX + boundary] 
            fatText=text.copy()
            for i in range(3):
                fatText=np.append(fatText,text,1)
            #text = cv2.cvtColor(text.astype(np.uint8), cv2.COLOR_BGR2GRAY)
            ...
            if sortedPartVotes:
                textRecongized=sortedPartVotes[0][0]
                textcache[textRecongized] = [startX, startY, endX, endY]
            cv2.rectangle(forTD, (startX, startY), (endX, endY), color, width)
            # draw the bounding box on the image
